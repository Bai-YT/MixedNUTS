...
class ResNetV2(nn.Module):
    ...
    def __init__(
        self, block_units, width_factor, head_size=21843, zero_head=False, return_features=False
    ):
        super().__init__()
        wf = width_factor  # shortcut 'cause we'll use it a lot.
        self.mean = nn.Parameter(torch.tensor([0.5, 0.5, 0.5]).view(3, 1, 1))
        self.std = nn.Parameter(torch.tensor([0.5, 0.5, 0.5]).view(3, 1, 1))
    ...
    def forward(self, x):
        ...
        out = self.body[2](feat2)
        out = self.body[3](out)
        out = self.head(out)
        assert out.shape[-2:] == (1, 1)  # We should have no spatial shape left.
        out = out[..., 0, 0]
        return (out, (feat1, feat2)) if self.return_features else out
