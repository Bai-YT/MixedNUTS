...
from models.output_maps import IdentityMap, HardMaxMap, LNClampPowerScaleMap
...
def run_robustbench(
    dataset_name, root_dir, rob_model_name, std_model_arch, map_type, beta,
    ln_k, n_examples, batch_size_per_gpu, device
):
    ...
    with open('optimal_spca.yaml', 'r') as file:
    ...
    forward_settings = {
        ...
        "std_map": std_map, "rob_map": rob_map,
    ...
    # Set data preprocessing transformation for ImageNet
    ...
    clean_acc, attacked_acc = benchmark(
        mix_model, model_name=model_full_name, threat_model=threat_model, eps=eps,
        dataset=dataset_name, data_dir=f"data/{dataset_name}",
        n_examples=n_examples, batch_size=min(batch_size, n_examples),
        preprocessing=transform, to_disk=True, device=torch.device(device)
    )
    print(f"Clean accuracy: {clean_acc:.2%}.")
    print(f"AutoAttacked accuracy: {attacked_acc:.2%}.")
